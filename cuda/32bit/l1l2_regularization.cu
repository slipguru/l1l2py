 ...
 *
 */
__global__ void SoftThreshFISTA(float * d_precalc, float * d_nsigma, float * d_mu_s, float * d_tau_s, float * d_beta, float * d_aux_beta, float * d_beta_next, float * d_beta_diff, float * d_value, float * d_t, float * d_t_next) {
 ...
 *
 */
float _sigma(float * d_matrixT, int n, int p, float mu, cublasHandle_t handle) {
    ...
    if (p > n) {
        status = cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, n, n, p, &alpha, d_matrixT, p, d_matrixT, p, &beta, d_tmp, n);
        cusolverDnSgesvd_bufferSize(cs_handle, n, n, &Lwork);
    } else {
        status = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, p, p, n, &alpha, d_matrixT, p, d_matrixT, p, &beta, d_tmp, p);
    ...
    cs_status = cusolverDnSgesvd(cs_handle, 'A', 'A', n_tmp, n_tmp, d_tmp, n_tmp, d_S, d_U, n_tmp, d_VH, n_tmp, Work, Lwork, NULL, devInfo);
 ...
 *
 */
int l1l2_regularization_optimized(float * d_XT, float * d_Y, float * d_XTY, int n, int p, float mu, float tau, float * d_beta_in, float * d_beta_out, int * k_final, int kmax, float tolerance, int adaptive,  cublasHandle_t handle) {

    int debug = 0;

    struct timeval t0, t1, t2, t3, y4;
