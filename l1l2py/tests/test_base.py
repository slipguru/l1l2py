...
def test_prediction():    
    ...
    # Standard call
    model = MockModel(fit_intercept=True).train(X, y)
    assert_almost_equal(2., model.intercept)
    ...
    # Pre-centered X
    model = MockModel(fit_intercept=False).train(Xc, y)
    assert_almost_equal(0.0, model.intercept)
    assert_almost_equal([1, 1, 1], model.beta)
    ...
    model = MockModel(fit_intercept=False).train(Xm, y)
    assert_almost_equal(0.0, model.intercept)
...
def test_ridge():
    ...
    model = RidgeRegression(mu=0.0).train(X, y) # OLS
